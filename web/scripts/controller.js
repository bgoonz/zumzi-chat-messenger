...
mesiboLive.controller("roomController", [
    "$scope",
    "$window",
    "$compile",
    "$timeout",
    ...
    function($scope, $window, $compile, $timeout, $anchorScroll) {
        ...
        $scope.toggleSelfVideo = function() {
            ...
            else $scope.addTicker("You have unmuted your video");
        };
        ...
        $scope.toggleSelfAudio = function() {
            ...
            else $scope.addTicker("You have unmuted your audio");
        };
        ...
        $scope.getMessageStatusClass = function(m) {
            ...
            if (!isValidString(status_class)) return -1;
        ...
        $scope.getMessageStatusColor = function(m) {
            ...
            if ($scope.isReceived(m)) return "";

            var status = m.status;
            var status_color = getStatusColor(status);
            if (!isValidString(status_color)) return "";
        ...
        $scope.getParticipantFromMessage = function(m) {
            ...
            if (isValid(m.groupid) && m.groupid > 0) return; // defaults to group chat
        ...
        $scope.clickUploadFile = function(peer, groupid) {
            ...
            setTimeout(function() {
                ...
                send_file_button.setAttribute(
                    "onclick",
                    "getScope().sendFile('" + peer + "'," + groupid + ");"
                );
        ...
        $scope.isImageMsg = function(m) {
            ...
            if (!isValid(m)) return false;
            if (!$scope.isFileMsg(m)) return false;
        ...
        $scope.isMesiboConnected = function() {
            if ($scope.mesibo_connection_status == MESIBO_STATUS_ONLINE) return true;
        ...
        $scope.Mesibo_OnConnectionStatus = function(status) {
            ...
            if (MESIBO_STATUS_AUTHFAIL == status) {
                ...
                $scope.addTicker(
                    "Auth failed while connecting to mesibo. Exiting room.."
                );
            ...
            if (MESIBO_STATUS_ONLINE == status) {
                ...
                if (navbar) navbar.style.backgroundColor = "#00868b";

                $scope.addTicker("You are Online");
                MesiboLog("======> You are online");

                if (
                    isValid($scope.publisher) &&
                    !$scope.publisher.isPublished &&
                    $scope.room.publish == 1
                ) {
            ...
            if (MESIBO_STATUS_OFFLINE == status) {
                ...
                var navbar = document.getElementById("mesibo_live_main_navbar");
                if (navbar) navbar.style.backgroundColor = "#800000";
        ...
        $scope.Mesibo_OnMessage = function(m, data) {
            ...
            var ticker_message = "";
            ticker_message =
                "You have a new message from " + $scope.addressBook[m.peer];

            if (m.groupid && m.peer)
                ticker_message =
                "You have a new group message from " + $scope.addressBook[m.peer];
            ...
            MesiboLog(
                ...
                m.peer,
                m.groupid,
                isValidString(m.peer),
                isValid(m.groupid)
            );

            var popup_identifier =
                isValid(m.groupid) && m.groupid > 0 ? m.groupid : m.peer;

            if (isValid(popup_identifier)) {
                var popup_div = document.getElementById(
                    "popup_chat_" + popup_identifier
                );
                ...
                if ($scope.isHidden(popup_div)) {
                    ...
                    var participant_chat_icon = document.getElementById(
                        "chat_popup_participant_" + m.peer
                    );
        ...
        $scope.Mesibo_OnParticipants = function(all, latest) {
            ...
            for (var i in latest) {
                ...
                if (p.getType && p.getType() > 0)
                    $scope.addTicker(
                        p.getName() + " is sharing the screen-" + p.getType()
                    );
                else $scope.addTicker(p.getName() + " has entered the room");
            }
        };
        ...
        $scope.Mesibo_OnParticipantUpdated = function(all, p) {
            ...
            MesiboLog(
                "Mesibo_OnParticipantsUpdated",
                p.getName(),
                "talking",
                p.isTalking(),
                p.muteStatus(false, true),
                p.muteStatus(true, true)
            );
            ...
            if ($scope.expanded_video_selected) {
                if (getStreamId($scope.expanded_video_selected) == getStreamId(p)) {
                    ...
                    $scope.expanded_video_selected.attach(
                        "expanded-screen-video-" +
                        getStreamId($scope.expanded_video_selected),
                        $scope.on_attached,
                        100,
                        2
                    );
                ...
                if ($scope.streams[i] === p) {
                    ...
                    if (remote) {
                        ...
                        if (p.getType() > 0)
                            $scope.addTicker(
                                p.getName() +
                                " has stopped sharing the screen " +
                                getStreamName(p)
                            );
                        else $scope.addTicker(p.getName() + " has left the room");
                    }
                            ...
                            if ($scope.expanded_video_selected) {
                                ...
                                for (var i = 0; i < $scope.streams.length; i++) {
                                    if ($scope.streams[i] != $scope.expanded_video_selected) {
                                        $scope.streams[i].attach(
                                            "video-small-" + getStreamId($scope.streams[i]),
                                            $scope.on_attached,
                                            100,
                                            10
                                        );
                                        $scope.streams[i].isExpandedScreen = false;
                                        $scope.setStripVideoDimensions(
                                            $scope.streams[i],
                                            "video-small",
                                ...
                                for (var i = 0; i < $scope.streams.length; i++) {
                                    $scope.streams[i].attach(
                                        "video-" + getStreamId($scope.streams[i]),
                                        $scope.on_attached,
                                        100,
                                        50
                                    );
                                    $scope.setGridVideoDimensions(
                                        $scope.streams[i],
                                        "video",
                                        DEFAULT_ASPECT_RATIO
                                    );
        ...
        $scope.setGridMode = function(stream_count) {
            ...
            if ($scope.grid_mode > MAX_GRID_MODE) $scope.grid_mode = MAX_GRID_MODE;
            ...
            if (previous_grid_mode != $scope.isGridChange) isGridChange = true;
        ...
        $scope.updateStreams = function(p, iAudio, iVideo) {
            ...
            $scope.$applyAsync(function() {
                MesiboLog(
                    "connectStream updateStreams",
                    p,
                    document.getElementById("video-" + getStreamId(p))
                );
                $scope.connectStream(
                    null,
                    p,
        ...
        $scope.toggleNameDisplay = function() {
            ...
            if ($scope.display_names) $scope.display_names_placeholder = "*";
            else $scope.display_names_placeholder = "";
            $scope.refresh();
        };
        ...
        $scope.getSelectedStream = function(stream_index) {
            ...
            /** Review Logic **/
            if (stream_index > $scope.streams.length) return null;
        ...
        $scope.isExistingStream = function(stream) {
            for (var i = $scope.streams.length - 1; i >= 0; i--) {
                if ($scope.streams[i] == stream) return true;
            }
        ...
        $scope.getParticipantFromId = function(pSid) {
            if (!isValid(pSid) || !(pSid > 0)) return null;

            for (var i = $scope.participants.length - 1; i >= 0; i--) {
                var sid = getStreamId($scope.participants[i]);
                if (sid == pSid) return $scope.participants[i];
            }
        ...
        $scope.getStreamFromId = function(pSid) {
            ...
            for (var i = $scope.streams.length - 1; i >= 0; i--) {
                ...
                if (sid == pSid) return $scope.streams[i];
            }
        ...
        $scope.updateCurrentSelected = function(participant) {
            if (!isValid(participant)) return -1;
        ...
        $scope.selectStream = function(participant) {
            ...
            if (!isValid(participant)) return;

            if (!$scope.isExistingStream(participant)) return;
        ...
        $scope.toggleStreamVisibility = function(participant) {
            ...
            var selected_stream = $scope.getStreamFromId(getStreamId(participant));
            if (!isValid(selected_stream)) return -1;
        ...
        $scope.getVideoStatusColor = function(stream) {
            ...
            if (stream.muteStatus(true) == true) return "#FF6347";
        ...
        $scope.getVideoStatusClass = function(stream) {
            ...
            if (stream.getType() > 0 && stream.isLocal()) {
                if (stream.muteStatus(true)) return "fa fa-play-circle";
                else return "fa fa-pause-circle";
            }

            if (stream.muteStatus(true)) return "fas fa-video-slash";
        ...
        $scope.getStatusIndicator = function(status) {
            ...
            if (!isValid(status)) return status_color;
            switch (status) {
        ...
        $scope.getBodyPadding = function() {
            var padding =
                ($scope.ultra_full_screen_mode ?
                    BODY_PADDING_FULLSCREEN :
                    BODY_PADDING_NORMAL) + "px";
            return padding;
        };
        ...
        $scope.isConnected = function(status) {
            ...
            if (MESIBO_STATUS_ONLINE == status) return true;
        ...
        function getElementWidth(ele_id) {
            if (!isValidString(ele_id)) return -1;
            var elem = document.getElementById(ele_id);
            if (!isValid(elem)) return -1;
        ...
        $scope.visibleHeight = function(element) {
            ...
            if (!(window && window.innerHeight)) return -1;
        ...
        $scope.setExpandedScreenDimensions = function() {
            ...
            document.getElementById("horizontal-strip-body").style.height =
                MAX_BOTTOM_STRIP_HEIGHT + "px";
        ...
        $scope.toggleExpandScreen = function(selected_stream) {
            ...
            MesiboLog(
                "toggleExpandScreen",
                "selected_stream.isExpandedScreen",
                selected_stream.isExpandedScreen
            );
            if (!selected_stream.isExpandedScreen) {
                //For hiding full screen
                $scope.closeFullScreen();
                return 0;
            }
            ...
            $scope.$applyAsync(function() {
                MesiboLog(
                    "toggleExpandScreen",
                    "Attaching fullscreen to:",
                    getStreamId($scope.expanded_video_selected),
                    $scope.expanded_video_selected,
        ...
        $scope.placePopupInGrid = function(popup_div, popup_identifier) {
            if (!isValid(popup_div) || !isValid(popup_identifier)) return null;
            ...
            var shift_bottom =
                10 + POPUP_HEIGHT * Math.floor(popup_grid_pos / 3) + "px";
            MesiboLog(
                "Dynamic postioning of popup",
                popup_grid_pos,
                shift_right,
                shift_bottom
            );
        ...
        $scope.sessionReadMessages = function(session_key, user, count) {
            ...
            MesiboLog(
                "sessionReadMessages ",
                session_key,
                peer + " " + " groupid " + groupid + " " + count
            );

            $scope.messageSession[session_key] = $scope.mesibo.readDbSession(
                peer,
                groupid,
                null,
                function on_read(count) {
                    ...
                    if (isValid(groupid) && groupid > 0) $scope.scrollToLastMsg(groupid);

                    if (isValidString(peer)) $scope.scrollToLastMsg(peer);
                }
            );
        ...
        $scope.showPopupChat = function(participant) {
            ...
            var participant_chat_icon = document.getElementById(
                "chat_popup_participant_" + popup_identifier
            );
            ...
            if (isValid(popup_div)) {
                ...
                if (!$scope.isHidden(popup_div)) {
                    ...
                    if (
                        !isValid($scope.messageSession) ||
                        !isValid($scope.messageSession[popup_identifier])
                    ) {
            ...
            popup_clone.getElementsByClassName("Messenger_header")[0].id =
                "popup_chat_" + popup_identifier + "_header";

            popup_clone.getElementsByClassName("popup_display_name")[0].id =
                "popup_display_name_" + popup_identifier;
            popup_clone.getElementsByClassName("popup_display_name")[0].innerHTML =
                selected.name;

            popup_clone
                .getElementsByClassName("popup-message-area")[0]
                .setAttribute(
                    "ng-repeat",
                    "m in messageSession['" + popup_identifier + "'].getMessages()"
                );
            popup_clone
                .getElementsByClassName("popup-message-area-1")[0]
                .setAttribute(
                    "ng-class",
                    "{'outgoing_msg': m && isSent(m),'incoming_msg':m && isReceived(m)}"
                );

            popup_clone
                .getElementsByClassName("popup-message-area-2")[0]
                .setAttribute(
                    "ng-class",
                    "{'sent_msg':isSent(m), 'received_msg':isReceived(m)}"
                );

            popup_clone
                .getElementsByClassName("popup-message-area-file")[0]
                .setAttribute("ng-if", "isFileMsg(m)");
            popup_clone
                .getElementsByClassName("popup-message-area-file")[0]
                .setAttribute("ng-href", "{{isFileMsg(m)? m.fileurl:''}}");

            popup_clone
                .getElementsByClassName("popup-message-area-image")[0]
                .setAttribute("ng-if", "isImageMsg(m)");
            popup_clone
                .getElementsByClassName("popup-message-area-image-src")[0]
                .setAttribute("ng-src", "{{m.fileurl}}");
            popup_clone
                .getElementsByClassName("popup-message-area-video")[0]
                .setAttribute("ng-if", "isVideoMsg(m)");
            popup_clone
                .getElementsByClassName("popup-message-area-video-src")[0]
                .setAttribute("ng-src", "{{m.fileurl}}");
            popup_clone
                .getElementsByClassName("popup-message-area-audio")[0]
                .setAttribute("ng-if", "isAudioMsg(m)");
            popup_clone
                .getElementsByClassName("popup-message-area-audio-src")[0]
                .setAttribute("ng-src", "{{m.fileurl}}");
            popup_clone
                .getElementsByClassName("popup-message-area-other")[0]
                .setAttribute("ng-if", "isOtherMsg(m)");

            popup_clone.getElementsByClassName(
                "popup-message-area-message-text"
            )[0].innerHTML = "{{m.title ? m.title : m.message}}";
            popup_clone
                .getElementsByClassName("popup-message-area-sender-name")[0]
                .setAttribute("ng-show", "m.groupid!=0 && isReceived(m)");
            popup_clone.getElementsByClassName(
                "popup-message-area-sender-name"
            )[0].innerHTML = "{{addressBook[m.peer]}}";
            popup_clone.getElementsByClassName(
                "popup-message-area-message-time"
            )[0].innerHTML = "{{m.date.time}} &nbsp;";

            popup_clone
                .getElementsByClassName("popup-message-area-message-time")[0]
                .setAttribute("ng-style", "{'float': isSent(m) ? 'left':'right'}");
            popup_clone
                .getElementsByClassName("popup-message-area-details-1")[0]
                .setAttribute(
                    "ng-style",
                    "{'margin-left': isReceived(m) ? '0!important':'0'}"
                );

            MesiboLog(popup_clone);
            popup_clone
                .getElementsByClassName("popup-message-area-message-status")[0]
                .setAttribute("ng-class", "getMessageStatusClass(m)");
            popup_clone
                .getElementsByClassName("popup-message-area-message-status")[0]
                .setAttribute("ng-style", "{'color':getMessageStatusColor(m)}");
            ...
            popup_clone
                .getElementsByClassName("popup-message-area-message-input")[0]
                .setAttribute(
                    "ng-keydown",
                    "$event.keyCode === 13 && sendMessage('" +
                    selected.peer +
            ...
            popup_clone
                ...
                .setAttribute(
                    "id",
                    "popup_message_area_message_input_" + popup_identifier
                );
            popup_clone
                .getElementsByClassName("popup-message-area-message-input")[0]
                .setAttribute(
                    "ng-model",
                    "input_message_text['" + popup_identifier + "']"
                );

            popup_clone
                .getElementsByClassName("popup-message-area-message-input-send")[0]
                .setAttribute(
                    "ng-click",
                    "sendMessage('" + selected.peer + "'," + selected.groupid + ")"
                );

            popup_clone
                .getElementsByClassName("popup-message-area-upload-file")[0]
                .setAttribute(
                    "ng-click",
                    "clickUploadFile('" + selected.peer + "'," + selected.groupid + ")"
                );

            popup_clone.getElementsByClassName(
                "popup-message-area-anchor-end"
            )[0].id = "messages_end_" + popup_identifier;
            ...
            popup_clone.getElementsByClassName("chat_close")[0].id =
                "mesibo_popup_" + popup_identifier;
            popup_clone
                .getElementsByClassName("chat_close")[0]
                .setAttribute(
                    "ng-click",
                    "closePopupChat('" + "mesibo_popup_" + popup_identifier + "')"
                );

            MesiboLog("reading messageSession for", selected.peer, selected.groupid);
            if (!isValid(selected.peer)) selected.peer = "";
            ...
            makePopupDraggable(
                document.getElementById("popup_chat_" + popup_identifier)
            );
        };

        $scope.closePopupChat = function(popup_class) {
            if (!isValidString(popup_class)) return -1;
            ...
            MesiboLog(
                "=====> closePopupChat, disabling read receipt",
                popup_identifier,
                $scope.messageSession[popup_identifier]
            );
            $scope.messageSession[popup_identifier].enableReadReceipt(false);
            $scope.freePopupGridPosition(popup_identifier);

            MesiboLog("closePopupChat", popup_class);
            if (!isValid(popup_class)) return;

            if (
                !$scope.isHidden(
                    document.getElementById("mesibo_popup_" + popup_identifier)
                )
            ) {
                $(".mesibo_popup_" + popup_identifier).toggle();
            }

            if ($scope.active_popup_chat_count > 0) $scope.active_popup_chat_count--;
        };
        ...
        $scope.sendMessage = function(peer, groupid) {
            ...
            if (!isValidString(peer) && !isValid(groupid)) return; //Neither a valid one-to-one to group message
            ...
            if (!isValidString(value)) return -1;
            ...
            if (isValidString(peer)) m.peer = peer;
            m.groupid = groupid;
            ...
            MesiboLog(
                "clear input",
                'document.getElementById("popup_message_area_message_input_' +
                popup_identifier +
                '").value = ""'
            );
            var pInput = document.getElementById(
                "popup_message_area_message_input_" + popup_identifier
            );
            $scope.refresh();
        };
        ...
        $scope.getLocalScreen = function(screen_id) {
            if (!isValid(screen_id) || screen_id <= 0) return null;

            var screen = null;

            for (var i = $scope.local_screens.length - 1; i >= 0; i--) {
                screen = $scope.local_screens[i];
                if (screen.getType() === screen_id) return screen;
            }

            //No existing screen with that id, create & return a new one
            screen = $scope.live.getLocalParticipant(
                screen_id,
                $scope.user.name,
                $scope.user.address
            );
            if (!isValid(screen)) return null;
        ...
        $scope.openScreenSharePreview = function(screen) {
            if (!(screen && screen.attach && screen.getType)) return;

            if (!(screen.getType() > 0)) return;

            $("#screenSharePreview").modal("show");
            document.getElementById("screen-share-id").innerHTML =
                "Sharing Screen-" + screen.getType();
        ...
        $scope.callParticipant = function(participant, audio, video) {
            ...
            if (!isValid(audio) || !isValid(video) || !isValid(participant)) return;
        ...
        $scope.$on("onExpandedScreenRendered", function(e) {
            ...
            $scope.setExpandedVideoDimensions(
                $scope.expanded_video_selected,
                "expanded-screen-video",
                DEFAULT_ASPECT_RATIO
            );
        ...
        $scope.setGridVideoDimensions = function(
            stream,
            stream_id_string,
            aspect_ratio
        ) {
            if (
                !isValid(stream) ||
                !isValid(stream_id_string) ||
                !isValid(aspect_ratio) ||
                aspect_ratio < 0
            )
                return;
            ...
            if ($scope.gri)
                MesiboLog(
                    "setGridVideoDimensions",
                    stream_id_string + "-" + getStreamId(stream),
                    adjusted_height
                );
            if (!adjusted_height) {
                ErrorLog(
            ...
            var stream_container = document.getElementById(
                stream_id_string + "-container-" + getStreamId(stream)
            );
            MesiboLog("setGridVideoDimensions", stream_container);
            if (!isValid(stream_container)) {
                ErrorLog(
                    "setGridVideoDimensions",
                    "Invalid Stream container",
                    stream_container
                );
                return;
            }
        ...
        $scope.setExpandedVideoDimensions = function(
            stream,
            stream_id_string,
            aspect_ratio
        ) {
            ...
            var adjusted_width = document.getElementById("expanded-screen-container")
                .style.width;
            var adjusted_height = document.getElementById("expanded-screen-container")
                .style.height;
        ...
        $scope.setStripVideoDimensions = function(
            stream,
            stream_id_string,
            aspect_ratio
        ) {
            ...
            stream_container.style.height = adjusted_height + "px";
            stream_container.style.width =
                adjusted_height * DEFAULT_ASPECT_RATIO + "px";
        };
        ...
        $scope.initPublisherHeight = function() {
            ...
            var publisher_height =
                document.getElementById("video-publisher-container").offsetWidth /
                DEFAULT_ASPECT_RATIO;
            ...
            document.getElementById("video-publisher").style.height =
                publisher_height + "px";
        };
        ...
        $scope.redrawGrid = function(orig_w) {
            ...
            MesiboLog(
                "=======>$scope.redrawGrid: ",
                $scope.ultra_full_screen_mode,
                $scope.ultra_full_screen_mode && w < MAX_STREAM_AREA_WIDTH,
                !$scope.ultra_full_screen_mode && w > MIN_STREAM_AREA_WIDTH
            );
            MesiboLog(
                w < MAX_STREAM_AREA_WIDTH,
                w > MIN_STREAM_AREA_WIDTH,
                MAX_STREAM_AREA_WIDTH,
                MIN_STREAM_AREA_WIDTH
            );
            if (
                ($scope.ultra_full_screen_mode && w < MAX_STREAM_AREA_WIDTH) ||
                (!$scope.ultra_full_screen_mode && w > MIN_STREAM_AREA_WIDTH)
        ...
        $scope.onFullScreenChange = function() {
            ...
            if (document.fullscreenElement) {
                ...
                MesiboLog(
                    `Element: ${document.fullscreenElement.id} entered full-screen mode with stream`
                );
        ...
        $scope.init = function() {
            ...
            [
                ...
                document.addEventListener(eventType, $scope.onFullScreenChange, false)
            );

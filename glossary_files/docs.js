...
function hr() {
	outputLetNav.push('<div style="background-color: #FFFFFF; border: 1px solid #ccc; height: 1px; margin: 5px auto 5px auto; width: 95%;"> </div>');
}
...
function walkTree(tree)
{
	for (var j=0;j<tree.length;j++)
	{
		...
		if (tree[j].section)
		{
			...
			outputLetNav.push('<li><a  "test=p" href="' + jsvar_baseurl + tree[j].path + '"')
...
function addSection(t) {
	var title = t.title;
	if(undefined != t.vtitle)
		title = t.vtitle;
	outputLetNav.push('<li><a  href="' + jsvar_baseurl + t.path + '"')
	outputLetNav.push('>'+title+'</a></li>')
}

function getProperty(o, p, d) {
...
function renderNav(docstoc) {
	...
	for (i=0;i<docstoc.horizontalnav.length;i++) {
		...
		var hide = getProperty(docstoc.horizontalnav[i], 'hide', false);
		var resetnav = getProperty(docstoc.horizontalnav[i], 'resetnav', false);
		resetnav = false;
		...
		if (docstoc.horizontalnav[i].node != "glossary")
		{
			...
			//mesibo
			if(!docstoc.hasOwnProperty(currentSection)) {
				addSection(docstoc.horizontalnav[i]);
				continue;
			}

			var treenode = docstoc[currentSection];
			//console.log(currentSection + " - " + hide + " - " + resetnav);
			//console.log(outputLetNav);
			// build vertical nav
			var itsHere = findMyTopic(treenode);

			if (itsHere || docstoc.horizontalnav[i].path == pageURL) {

				if(resetnav == 'yes' || resetnav == true) {
					outputLetNav = new Array();	

					// add default homes menu after reset
					walkTree(docstoc.homes);
				}
				...
				var printhr = 0;
				if(treenode.length > 1 || treenode[0].hasOwnProperty("section"))
					printhr = 1;

				if(printhr) hr();
				//do not add hr() inside walkTree as it's a recursive function
		...
		sectionToHighlight = false; //mesibo, this create multiple highlights and hence disabled
		if (docstoc.horizontalnav[i].path == pageURL || docstoc.horizontalnav[i].node == sectionToHighlight) {
			outputHorzTabs.push(' class="active"');
		}
		outputHorzTabs.push('><a href="'+jsvar_baseurl + docstoc.horizontalnav[i].path+'">'+docstoc.horizontalnav[i].title+'</a></li>\n');
	}
	...
	if (outputLetNav.length==0)
	{
		...
		for (var i=0;i<glossary.length;i++)
		{
			...
			outputLetNav.push('<li><a'+highlightGloss+' href="' + jsvar_baseurl + '/documentation/glossary/?term=' + glossary[i].term + '">'+glossary[i].term+'</a></li>');
		}
	}
...
{
	if (document.location.pathname.indexOf("/documentation/glossary/")<0){
